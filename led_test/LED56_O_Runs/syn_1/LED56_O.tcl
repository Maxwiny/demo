...
set rc [catch {
  ...
  elaborate -top {top}
  export_db {LED56_O_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../io.adc
  optimize_rtl
  report_area -file LED56_O_rtl.area
  export_db {LED56_O_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea LED56_O_gate.area
  legalize_phy_inst
  export_db {LED56_O_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
