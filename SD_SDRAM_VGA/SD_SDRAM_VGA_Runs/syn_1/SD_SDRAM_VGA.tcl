...
set rc [catch {
  open_project {SD_SDRAM_VGA.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {hdmi_colorbar}
  export_db {SD_SDRAM_VGA_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../io.adc
  optimize_rtl
  report_area -file SD_SDRAM_VGA_rtl.area
  export_db {SD_SDRAM_VGA_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -maparea SD_SDRAM_VGA_gate.area
  legalize_phy_inst
  export_db {SD_SDRAM_VGA_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
