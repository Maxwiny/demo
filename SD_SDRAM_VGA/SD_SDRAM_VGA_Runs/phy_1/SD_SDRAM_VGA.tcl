...
set rc [catch {
  ...
  import_db {../syn_1/SD_SDRAM_VGA_gate.db}
  place
  export_db {SD_SDRAM_VGA_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file SD_SDRAM_VGA_phy.area
  export_db {SD_SDRAM_VGA_pr.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid SD_SDRAM_VGA_inst.bid
  bitgen -bit "SD_SDRAM_VGA.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
